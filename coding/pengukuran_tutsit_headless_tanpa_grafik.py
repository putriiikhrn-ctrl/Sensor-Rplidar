        ...
        if is_real_mode and lidar:
            ...
            for scan in lidar.iter_scans(max_buf_meas=1000):
                ...
                if len(current_distances) >= 3:
                    ...
                    # 1. TAMPILKAN DASHBOARD DI TERMINAL
                    bersihkan_terminal()
                    print("==========================================================")
                    print("         SISTEM PENGUKURAN TUTSIT (REAL HEADLESS)         ")
                    print("==========================================================")
                    print(f" Status Sistem       : AKTIF (REAL-TIME)")
                    print(f" Port Sensor         : {config.port_name}")
                    print(f" Jumlah Titik Data   : {len(current_distances)} Pts")
                    print(f" Sudut Laser Terakhir: {sudut_terakhir:.1f}°")
                    print("----------------------------------------------------------")
                    print(" DIAMETER DALAM TANGKI (FITTED):")
                    print(f"  -> {diameter_mm:10.2f} mm")
                    print(f"  -> {diameter_cm:10.2f} cm")
                    print("----------------------------------------------------------")
                    print(" ANALISIS PERGESERAN SENSOR (OFF-CENTER):")
                    print(f"  -> Posisi Pusat X  : {x_c:10.1f} mm")
                    print(f"  -> Posisi Pusat Y  : {y_c:10.1f} mm")
                    print(f"  -> Total Pergeseran: {jarak_pergeseran_mm:10.1f} mm")
                    print("==========================================================")
            ...
            while True:
                ...
                bersihkan_terminal()
                print("==========================================================")
                print("         SISTEM PENGUKURAN TUTSIT (SIMULASI HEADLESS)     ")
                print("==========================================================")
                print(f" Status Sistem       : DEMO MODE (SIMULASI)")
                print(f" Koneksi Sensor      : TERPUTUS (DISCONNECTED)")
                print(f" Jumlah Titik Data   : 360 Pts (Generated)")
                print(f" Sudut Laser         : 360.0° (Loop)")
                print("----------------------------------------------------------")
                print(" DIAMETER ESTIMASI (FITTED):")
                print(f"  -> {diameter_mm:10.2f} mm  [Error Fitting: {error_fitting_baru:.2f} mm]")
                print(f"  -> {diameter_cm:10.2f} cm")
                print("----------------------------------------------------------")
                print(" ESTIMASI METODE LAMA (RATA-RATA SIMPLE):")
                print(f"  -> {diameter_rata_rata_lama:10.2f} mm  [Error Rata2 : {error_rata_rata_lama:.2f} mm]")
                print("----------------------------------------------------------")
                print(" IDENTIFIKASI PERGESERAN ALAT:")
                print(f"  -> Posisi Pusat X  : {x_c_fit:10.1f} mm")
                print(f"  -> Posisi Pusat Y  : {y_c_fit:10.1f} mm")
                print(f"  -> Total Pergeseran: {jarak_pergeseran_fit:10.1f} mm")
                print("==========================================================")
                print(" Tekan Ctrl+C untuk menghentikan program.")
                ...
                time.sleep(0.4) # Beri jeda agar pembacaan terminal & LCD nyaman dilihat
